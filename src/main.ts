...
//Minio服务器地址
let minioUrl = 'http://localhost:9000/hxoa';
app.config.globalProperties.$minioUrl = minioUrl;

//配置WebSocket
// import VueNativeSock from 'vue-native-websocket-vue3';
//
// app.use(VueNativeSock, 'ws://localhost:6101/his-api/socket', {
//     format: 'json',
//     reconnection: true
// });
...
/********封装全局Ajax公共函数********************/
app.config.globalProperties.$http = function (url : string, method : string, data : any, async : boolean, fun : Function) {
	if (data != null) {
		for (let [key, value] of Object.entries(data)) {
			//不能使用==，它会把false==''当做成立条件
			if (value === '') {
				data[key] = null;
			}
		}
...
/********将Base64字符串转换为Blob对象********************/
app.config.globalProperties.$base64ToBlob = function (base64Data : any) {
...
/********封装全局Ajax上传文件公共函数********************/
app.config.globalProperties.$upload = function (url : string, data : FormData, async : boolean, fun : Function) {
	if (data == null) {
		ElMessage.error({
			message: "上传文件不能为空",
			duration: 1200
...
app.config.globalProperties.getPdf = function (id : string, filename : String) {
	...
	let imageWrapper = document.querySelector('#' + id); // 获取DOM
